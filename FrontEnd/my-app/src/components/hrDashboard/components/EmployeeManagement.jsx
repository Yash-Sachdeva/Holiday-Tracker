...
const EmployeeManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingEmployee
        ? 'http://localhost:8080/hr/update-employee'
        : 'http://localhost:8080/hr/register-employee';
  ...
  const handleDelete = async (employeeId) => {
    try {
      const response = await fetch(`http://localhost:8080/hr/delete-employee/${employeeId}`, {
        method: 'DELETE',
        credentials: 'include'
      });
