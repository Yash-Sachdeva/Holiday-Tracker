...
const HRManagement = () => {
  ...
  const fetchHRs = async () => {
    try {
      const response = await fetch('http://localhost:8080/admin/all-hrs', {
        credentials: 'include'
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingHR
        ? 'http://localhost:8080/admin/update-hr'
        : 'http://localhost:8080/admin/register-hr';
  ...
  const handleDelete = async (hr) => {
    try {
      const response = await fetch('http://localhost:8080/admin/delete-hr', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
